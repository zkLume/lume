...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("\n=== step 2: Ink — building Merkle tree ===\n");
    let mut ink = Ink::new();
    let leaves: Vec<&[u8]> = reports.iter().map(|(_, body)| body.as_bytes()).collect();
    ink.commit(&leaves);

    let root: Tip5Hash = ink.root().expect("committed");
    println!("  root: {:?}", root);
    ...
    // --- step 4: verify all proofs locally with Grip ---

    println!("\n=== step 4: Grip — local verification ===\n");
    let mut grip = Grip::new();
    grip.register_root(root);

    for (i, (title, body)) in reports.iter().enumerate() {
        let proof = ink.proof(i);
        let valid = grip.check(body.as_bytes(), &proof, &root);
        println!("  {} '{}': {}", if valid { "ok" } else { "FAIL" }, title, valid);
    }

    // --- step 5: demonstrate settlement via Beak ---
    //
    ...
    let tampered = grip.check(b"Revenue down 50%. CEO arrested.", &proof, &root);
    println!("  tampered report: valid={}", tampered);

    let bad_root: Tip5Hash = [0xDEAD, 0, 0, 0, 0];
    let unreg = grip.check(reports[0].1.as_bytes(), &proof, &bad_root);
    println!("  unregistered root: valid={}", unreg);

    println!("\n=== done ===");
    println!("\nThe Beak pattern: submit -> commit -> register -> verify -> settle.");
