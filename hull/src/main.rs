...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // --- [8] Self-verification (via Mantle Grip) ---
    println!("\n--- Self-verification (Grip) ---");
    let mut grip = vesl_mantle::Grip::new();
    grip.register_root(root);
    let all_valid = grip.check_manifest(&manifest, &root);
    ...
    println!("  Grip manifest check: {}", if all_valid { "PASSED" } else { "FAILED" });
