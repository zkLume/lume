::  lib/lume-logic.hoon: Merkle, manifest & settlement for Sovereign-RAG
::
...
::  Three-vessel clean: CLI, Urbit, TEE Cloud.
::
/=  *  /common/zeke
::
|%
...
::
::  The notary gate. Accepts a pending Lume Note, verifies the
...
::
++  settle-note
  |=  $:  current-note=[id=@ vessel=@ root=@ state=[%pending ~]]
          mani=[query=@t results=(list [chunk=[id=@ dat=@t] proof=(list [hash=@ side=?]) score=@ud]) prompt=@t output=@t]
          expected-root=@
      ==
  ^-  [id=@ vessel=@ root=@ state=[%settled ~]]
  ?>  (verify-manifest mani expected-root)
  [id=id.current-note vessel=vessel.current-note root=root.current-note state=[%settled ~]]
--
