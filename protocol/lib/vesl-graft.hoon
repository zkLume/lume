::  lib/vesl-graft.hoon: composable Vesl state for any NockApp kernel
::
...
::
::  Import this into your kernel, compose vesl-state into your
::  versioned-state, and delegate tagged pokes to vesl-poke.
::  No wrapper, no NockApp lifecycle, no tx-engine. Just the
::  verification and settlement logic, ready to graft.
::
::  Usage:
::    /-  *vesl
::    /+  *vesl-graft
...
::    ...in poke arm...
::    ?+  -.cause  [~ state]
::      %vesl-register  (vesl-poke vesl.state cause)
::      %vesl-settle    (vesl-poke vesl.state cause)
::      %vesl-verify    (vesl-poke vesl.state cause)
::    ==
::
/-  *vesl
/+  *vesl-logic
...
::
::  Returns [effects updated-state]. Caller integrates the
::  updated vesl-state back into their kernel state.
::
++  vesl-poke
  |=  [state=vesl-state cause=vesl-cause]
  ...
  ?-  -.cause
    ...
    ::
    ::  %vesl-settle — cue payload, validate, settle note
    ::    Guards: root must be registered, note must not be settled
    ::
      %vesl-settle
    ...
    ::  Settle — crashes on verification failure (?>)
    ::
