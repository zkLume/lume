...
//! Lume coordinates via the wallet's private gRPC API:
//!
//! 1. **Peek** — query wallet balance and available UTXOs
//! 2. **Poke** — request transaction creation with Lume settlement context
//!
//! # Transaction Flow
//!
//! ```text
//! Lume                           Wallet (private gRPC)
//!  |                               |
...
impl WalletClient {
    ...
    ///
    /// 1. Lume constructs an unsigned transaction with Lume NoteData
    /// 2. Lume computes the transaction hash
    /// 3. Lume asks the wallet to sign the hash (this method)
    /// 4. Lume retrieves the signature from wallet effects
    /// 5. Lume applies the signature and submits via ChainClient
    ...
    ///
    /// The settlement data is logged by the vessel for local tracking.
    ...
    /// by the vessel but not embedded in the on-chain note. Full NoteData
