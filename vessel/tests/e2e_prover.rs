...
//!
//! Proves that the Rust Vessel and Hoon ZK-circuit produce identical data:
//!
...
fn rust_payload_structure_integrity() {
    ...
    // --- Verify note structure: [id=42 [vessel=7 [root [%pending 0]]]] ---
    ...
    let vessel = note.slot(6).expect("vessel").as_atom().expect("vessel atom").as_u64().expect("vessel u64");
    assert_eq!(vessel, 7, "vessel = 7");
    ...
    eprintln!("  Note: id=42, vessel=7, state=%pending, root=256bit");
